...
bool Client::SendVector(sf::Vector2f &_vector)
{
	if (!SendPacketType(P_Vector)) //Send packet type: Chat Message, If sending packet type fails...
		return false; //Return false: Failed to send string
	if (!sendall((char*)&_vector, sizeof(sf::Vector2f))) //Try to send int... If int fails to send
		return false; //Return false: int not successfully sent
	return true; //Return true: int successfully sent
}

bool Client::GetVector(sf::Vector2f& _vector)
{
	if (!recvall((char*)&_vector, sizeof(sf::Vector2f))) //Try to receive int... If int fails to be recv'd
		return false; //Return false: Int not successfully received
	return true;//Return true if we were successful in retrieving the int
}
