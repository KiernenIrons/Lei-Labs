...
bool Client::ProcessPacket(Packet _packettype)
{
	...
	case P_Vector: //If packet is a position packet
	{
		sf::Vector2f _vector; //vector to store our position we received
		if (!GetVector(_vector)) //Get the position and store it in variable: position
			return false; //If we do not properly get the position, return false
		m_vector = _vector; //Set users to the position
		break;
	}
