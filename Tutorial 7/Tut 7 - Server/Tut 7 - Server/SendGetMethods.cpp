...
{
	if (!SendPacketType(ID, P_Vector)) //Send packet type: Chat Message, If sending packet type fails...
		return false; //Return false: Failed to send string
	if (!sendall(ID, (char*)&_vector, sizeof(sf::Vector2f))) //Try to send float... If float fails to send
		return false; //Return false: float not successfully sent
	return true; //Return true: float successfully sent
}

bool Server::GetVector(int ID, sf::Vector2f& _vector)
{
	if (!recvall(ID, (char*)&_vector, sizeof(sf::Vector2f))) //Try to receive float... If float fails to be recv'd
		return false; //Return false: float not successfully received
	return true;//Return true if we were successful in retrieving the float
}
