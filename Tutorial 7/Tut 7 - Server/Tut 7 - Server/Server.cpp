...
bool Server::ListenForNewConnection()
{
	...
	else //If client connection properly accepted
	{
		...
		std::string MOTD = "MOTD: Welcome! Suck my ass!.";
...
bool Server::ProcessPacket(int ID, Packet _packettype)
{
	...
	case P_Vector: //Packet Type: float
	{
		sf::Vector2f _vector; //string to store our float we received
		if (!GetVector(ID, _vector)) //Get the float and store it in variable: _float
			return false; //If we do not properly get the float, return false

		std::cout << _vector.x << ", " << _vector.y << std::endl;
		//Next we need to send the float out to each user
		for (int i = 0; i < TotalConnections; i++)
		{
			if (i == ID) //If connection is the user who sent the float...
				continue;//Skip to the next user since there is no purpose in sending the float back to the user who sent it.
			if (!SendVector(i, _vector)) //Send message to connection at index i, if message fails to be sent...
			{
